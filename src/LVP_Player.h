...
namespace LibVoyaPlayer
{
	namespace MediaPlayer
	{
		...
		private:
			...
			static void                             createAudioBuffer(int bufferSize);
			static void                             decodeAudio(uint8_t* stream, int& streamSize, LibFFmpeg::AVFrame* frame = NULL);
			static int                              decodeAudioFrame(LibFFmpeg::AVFrame* frame);
			static void                             decodeAudioPacket(LibFFmpeg::AVPacket* packet);
			static std::vector<LibFFmpeg::AVFrame*> getAudioFrames();
			static LibFFmpeg::AVPacket*             getAudioPacket();
			...
			static void                             initAudioFilter(LibFFmpeg::AVFrame* frame);
			...
			static void                             seekTo(double percent);
			static void                             setAudioProgress(LibFFmpeg::AVPacket* packet, LibFFmpeg::AVFrame* frame);
			static void                             stop(const std::string& errorMessage = "");
			static void                             threadAudio(void* userData, uint8_t* stream, int streamSize);
			static int                              threadPackets();
			static int                              threadSub();
			static int                              threadVideo();
			static void                             updateAudioDevice();
		};
	}
}
